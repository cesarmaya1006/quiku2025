...
use App\Models\Personas\UsuarioTemp;
use App\Models\User;
use Carbon\Carbon;
...
class Tabla_Usuarios extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $usuario1 = User::create([
            'name' => 'adminsis',
            'email' => 'adminsis@mgl.com',
            'password' => bcrypt('123456789')
        ])->syncRoles('Administrador Sistema', 'Super Administrador', 'Administrador', 'Coordinador', 'Funcionario', 'Usuario', 'Analitica', 'Wiku');
        // -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
        ...
        $usuario3 = User::create([
            'name' => 'usuario',
            'email' => 'usuario@mgl.com',
            'password' => bcrypt('123456789')
        ])->syncRoles('Usuario');
        $usuarioTemp = UsuarioTemp::create([
            'tipo_persona' => 2,
            'docutipos_id' => 1,
        ...
        foreach ($empleados as $empleado) {
            ...
            $nuevafecha = strtotime ('-'.$annos.' year' , strtotime($fechaactual)); //Se resta un año menos
            $nuevafecha = date ('Y-m-d',$nuevafecha);

            $nuevafecha = strtotime ('-'.$dias.' day' , strtotime($nuevafecha)); //Se resta un año menos
            $nuevafecha = date ('Y-m-d',$nuevafecha);


            $usuarioArray = User::create([
